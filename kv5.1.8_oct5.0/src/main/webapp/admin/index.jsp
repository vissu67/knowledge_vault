...
<head>
  ...
  <title>Knowledge Vault Administration</title>
...
<% com.openkm.api.OKMAuth.getInstance().login(); %>
  ...
  <c:choose>
    <c:when test="${isGroupAdmin}">
	  <div id="menu"><%@include file="groupmenu.jsp" %></div>
	  <div><iframe id="frame" name="frame" src="Auth?action=groupUserList" frameborder="0"></iframe></div>
    </c:when>
    <c:otherwise>
      <div class="error"></div>
    </c:otherwise>
